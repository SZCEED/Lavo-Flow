...
// ==========================================
// 1. DOKUMEN LEGAL (SAYA TAMBAHKAN INI AGAR TOMBOL BERFUNGSI)
// ==========================================
const legalDocs = {
  terms: `SYARAT & KETENTUAN (TERMS)

1. PENGGUNAAN LAYANAN
Selamat datang di Lavo. Aplikasi ini dilisensikan kepada Anda untuk penggunaan bisnis.

2. PEMBAYARAN
Layanan ini berbasis langganan prabayar. Kode lisensi yang sudah diaktifkan tidak dapat diuangkan kembali (non-refundable).

3. TANGGUNG JAWAB
Kami tidak bertanggung jawab atas kesalahan input data yang dilakukan oleh pengguna.`,

  privacy: `KEBIJAKAN PRIVASI (PRIVACY)

1. DATA PENGGUNA
Kami menyimpan data nama, nomor telepon, dan transaksi bisnis Anda di server terenkripsi (Supabase).

2. PENGGUNAAN DATA
Data hanya digunakan untuk fungsionalitas aplikasi dan tidak akan dijual ke pihak ketiga.`,

  eula: `PERJANJIAN LISENSI (EULA)

1. LISENSI
Anda diberikan hak non-eksklusif untuk menggunakan perangkat lunak ini.

2. LARANGAN
Dilarang keras membongkar (reverse engineer), memodifikasi, atau menjual ulang kode sumber aplikasi ini.

3. TERMINASI
Kami berhak memutus akses jika terdeteksi pelanggaran.`
};

// ==========================================
// 2. DATABASE BAHASA & TEKS (KAMUS LENGKAP + TAMBAHAN SUBS)
// ==========================================
const resources = {
  ID: {
    // Navigasi
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-blue-100 text-slate-800 pb-20">
      ...
      {/* LEGAL MODAL (FIXED - MUNCULKAN ISI TEKS) */}
      {showLegal && (
          <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-8 animate-in fade-in">
              <div className="bg-white max-w-2xl w-full p-12 rounded-[40px] shadow-2xl relative">
                  <button onClick={() => setShowLegal(null)} className="absolute top-8 right-8 text-2xl hover:text-red-500">&times;</button>
                  <h2 className="text-2xl font-black mb-6 uppercase">{t(`legal_${showLegal}`)}</h2>
                  <div className="whitespace-pre-wrap text-sm text-slate-600 font-medium leading-relaxed max-h-[60vh] overflow-y-auto pr-2 border-t pt-4">
                      {legalDocs[showLegal] /* MENGAMBIL DATA DARI VARIABEL LEGALDOCS */}
                  </div>
                  <button onClick={() => setShowLegal(null)} className="mt-8 bg-slate-900 text-white px-6 py-3 rounded-xl w-full font-bold">{t('close')}</button>
              </div>
          </div>
      )}
